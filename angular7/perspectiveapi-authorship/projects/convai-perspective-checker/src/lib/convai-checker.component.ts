...
import {CommentFeedback, LoadingIconStyle, PerspectiveStatusComponent} from './convai-perspective-status.component';
...
export interface InputEvent { target: HTMLInputElement; }
...
@Component({
  ...
  // TODO: remove comment, once tested: moved to using HostLIstener.
  // host: {
...
export class ConvaiCheckerComponent implements OnInit, OnChanges {
  ...
  constructor(
      ...
      private analyzeApiService: ConvaiPerspectiveApiService) {
    ...
    const checkText$ =
        this.analyzeApiService
            ...
            .pipe(finalize(() => {
              ...
              // TODO: Add a comment to explain why/how
              // this.analyzeCommentResponse is set to the new value. That seems
              // to happen later in the subscribe.
              const newScore = this.getMaxScore(this.analyzeCommentResponse);
              // TODO: notifyScoreChange's behaviour is async: we should either
              // make it sync, or chain it so that the piped result completes
              // AFTER notifyScoreChange has compkleted.
